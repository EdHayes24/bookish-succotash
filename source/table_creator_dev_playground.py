...
def open_connection_wrapper():
    try:
        cnx = mysql.connector.connect(
            user="root", host="mydb", password="root", port=3306
        )
...
if db_name in db_names:
    ...
    usr_create = get_min_length_string(
...
if len(table_names) == 0:
    ...
    if usr_create.lower() == "y":
        ...
        curse.execute(
            f"CREATE TABLE {table_name} ({col1[0]} {col1[1]}, {col2[0]} {col2[1]}, {col3[0]} {col3[1]})"
        )
else:
    for i, table in enumerate(table_names):
        print(f"{i} = {table}")
    pick_table = get_min_length_string(
        "Select Table from above options to start playing with"
    )
...
if "orders" not in table_names:
    curse.execute(
        "CREATE TABLE orders (id INT AUTO_INCREMENT PRIMARY KEY, name VARCHAR(255), address VARCHAR(255), tel VARCHAR(15), status VARCHAR(15), time DATE, items VARCHAR(255), courier_id INT)"
    )
    curse.execute(
        "INSERT INTO orders (name, address, tel, status, time, items, courier_id) VALUES ('Tim', 'Here', '0800001066', 'preparing', 2022-11-21,'[chocolate, banana]', '1');"
    )
if "couriers" not in table_names:
    curse.execute(
        "CREATE TABLE couriers (id INT AUTO_INCREMENT PRIMARY KEY, name VARCHAR(255), tel VARCHAR(15), order_ids VARCHAR(255))"
    )
    curse.execute(
        "INSERT INTO couriers (name, tel, order_ids) VALUES ('courier_pete', '0800118118', '1');"
    )
# Temp file 2:
...
dtype_time = ("DATE", "DATETIME(fsp)", "TIMESTAMP(fsp)", "TIME(fsp)", "YEAR")
...
cnx = mysql.connector.connect(
    host="mydb", user="root", password="root", port=3306, database="db_playground"
)
print(cnx)
